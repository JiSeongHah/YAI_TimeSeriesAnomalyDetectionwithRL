...
class MainLoop():
    ...
    def validationStepEnd(self,iterNum):
        ...
        if iterNum%100 == 0:
            ...
            print(f'episode complete with TP : {np.sum(self.TPLstVal[-10:])}, FN : {np.sum(self.FNLstVal[-10:])}'
                  f', FP : {np.sum(self.FPLstVal[-10:])} , TN : {np.sum(self.TNLstVal[-10:])}')
            plt.close()
            plt.cla()
            plt.clf()
    ...
    def StartTrnAndVal(self,iterNum):
        ...
        for iteration in range(iterNum):
            print(f'{iteration}/{iterNum} training start...')
            t = time.time()
            self.trnOneEpisode()
            self.validateOneEpisode()
            self.validationStepEnd(iterNum=iterNum)
            print(f'{iteration}/{iterNum} training complete! and time elpased : {time.time()-t}')
